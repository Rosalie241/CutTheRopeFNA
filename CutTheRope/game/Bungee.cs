...
namespace CutTheRope.game
{
	internal class Bungee : ConstraintSystem
	{
		...
		private static void drawAntialiasedLineContinued(float x1, float y1, float x2, float y2, float size, RGBAColor color, ref float lx, ref float ly, ref float rx, ref float ry, bool highlighted)
		{
			...
			if (!MathHelper.vectEqual(vector, MathHelper.vectZero))
			{
				Vector v3 = (highlighted ? vector : MathHelper.vectMult(vector, ((double)color.a == 1.0) ? 1.02 : 1.0));
				...
				v7 = MathHelper.vectAdd(v7, v);
				Vector v8 = MathHelper.vectAdd(v4, v3);
				Vector v9 = MathHelper.vectAdd(v5, v3);
				Vector vector3 = MathHelper.vectMult(vector2, size + 6f);
				Vector v10 = MathHelper.vectNeg(vector3);
				Vector v11 = MathHelper.vectAdd(vector3, vector);
				Vector v12 = MathHelper.vectAdd(v10, vector);
				vector3 = MathHelper.vectAdd(vector3, v);
				v10 = MathHelper.vectAdd(v10, v);
				v11 = MathHelper.vectAdd(v11, v);
				v12 = MathHelper.vectAdd(v12, v);
				if (lx == -1f)
				{
				...
				v8 = MathHelper.vectAdd(v8, v);
				v9 = MathHelper.vectAdd(v9, v);
				lx = v6.x;
				ly = v6.y;
				rx = v7.x;
				ry = v7.y;
				Vector vector4 = MathHelper.vectSub(v4, vector2);
				Vector vector5 = MathHelper.vectSub(v8, vector2);
				Vector vector6 = MathHelper.vectAdd(v5, vector2);
				Vector vector7 = MathHelper.vectAdd(v9, vector2);
		...
		private static void drawBungee(Bungee b, Vector[] pts, int count, int points)
		{
			...
			float num3 = MathHelper.vectDistance(MathHelper.vect(pts[0].x, pts[0].y), MathHelper.vect(pts[1].x, pts[1].y));
